...
─────────────────────────
The EHS Engine acts as a concrete Observer, subscribing ONLY to the
'telemetry.enviro.#' topic on the RabbitMQ exchange.

Key properties of this Observer implementation:
  ✅ Selective subscription: ignores telemetry.power.* and telemetry.cameras.*
  ✅ Fire-and-forget consumption: RabbitMQ pushes messages; we don't poll
  ✅ Isolated failure: if EHS consumer crashes, the Energy/CAM engines continue
...
class AMQPConsumer:
    """
    AMQP consumer that listens on 'telemetry.enviro.#' and dispatches
    each message to the EHSEngineEvaluator for processing.
    """
    ...
    def __init__(
        ...
        queue_name: str = "ehs_telemetry_queue",
        binding_key: str = "telemetry.enviro.#",
    ):
    ...
    def _consume_loop(self):
        ...
        while True:
            try:
                ...
                # Declare our exclusive queue and bind to enviro topics ONLY
    ...
    def _on_message(self, channel, method, properties, body: bytes):
        """
        Callback invoked by RabbitMQ for each incoming message on telemetry.enviro.*
        
        1. Parse raw JSON body into EHSTelemetry Pydantic model.
        2. Hand off to EHSEngineEvaluator.evaluate().
        3. ACK the message so RabbitMQ removes it from the queue.
        """
        try:
            raw = json.loads(body.decode("utf-8"))

            # Parse the SmartCityObject payload into EHS-specific schema
            # The 'data' field contains aqi, water_ph from Member 1's EHSNode
